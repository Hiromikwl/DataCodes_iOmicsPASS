...
pdf("heatmap_EdgesSurv_noCNV.pdf",height=16,width=10, useDingbats = F)
...
outPCA = prcomp(t(as.matrix(Xnew)),center=T,scale =T)

expl.var <- round(outPCA $sdev^2/sum(outPCA$sdev^2),3)*100 
...
### highlighting subjects who has class probabilities < 0.5 in red color ###
oddS = colnames(Xsurv)[which(ClassProb<0.5)]


pdf("PCAplot_BRCAclustering_noCNV.pdf",height=6,width=12, useDingbats = F)
par(oma=c(2,1,2,1),mar=c(4,4,2,1),mfrow=c(1,2))
plot(PC1,PC2,cex =0.7,type="n",ylim=c(-50,50),xlim=c(-100,100),xlab=paste0("PC1 (", expl.var[1], "%)"),ylab=paste0("PC2 (", expl.var[2], "%)"),cex.lab = 0.8)
points(PC1,PC2,pch=pchCol,col=colvec, cex=1.3)
abline(h=0,lty=2)
abline(v=0,lty=2)
legend(-100,60, uniqS, xpd = TRUE, horiz = TRUE, inset = c(0, 0),
       bty = "n", pch = c(16,17,3,4),col=tmp_col,cex=0.8)
plot(PC1,PC3,cex =0.7,type="n",ylim=c(-50,50),xlim=c(-100,100),xlab=paste0("PC1 (", expl.var[1], "%)"),ylab=paste0("PC3 (", expl.var[3], "%)"),cex.lab = 0.8)
points(PC1,PC3,pch=pchCol,col=colvec,cex=1.3)
abline(h=0,lty=2)
abline(v=0,lty=2)
legend(-100,60, uniqS, xpd = TRUE, horiz = TRUE, inset = c(0, 0),
       bty = "n", pch = c(16,17,3,4),col=tmp_col,cex=0.8)
...
pdf("PCAplot_BRCAclustering_noCNV_withClassProb.pdf",height=6,width=12, useDingbats = F)
par(oma=c(2,1,2,1),mar=c(4,4,2,1),mfrow=c(1,2))
plot(PC1,PC2,cex =0.7,type="n",ylim=c(-50,50),xlim=c(-100,100),xlab=paste0("PC1 (", expl.var[1], "%)"),ylab=paste0("PC2 (", expl.var[2], "%)"),cex.lab = 0.8)
points(PC1,PC2,pch=pchCol,col=colvec,cex=1.3)
points(PC1[match(oddS,colnames(Xsurv))],PC2[match(oddS,colnames(Xsurv))]  ,pch=pchCol[match(oddS,colnames(Xsurv))],col=2,cex=1.3)
abline(h=0,lty=2)
abline(v=0,lty=2)
...
legend(-100,60, uniqS, xpd = TRUE, horiz = TRUE, inset = c(0, 0),
       bty = "n", pch = c(16,17,3,4),col=tmp_col,cex=0.8)
plot(PC1,PC3,cex =0.7,type="n",ylim=c(-50,50),xlim=c(-100,100),xlab=paste0("PC1 (", expl.var[1], "%)"),ylab=paste0("PC2 (", expl.var[3], "%)"),cex.lab = 0.8)
points(PC1,PC3,pch=pchCol,col=colvec,cex=1.3)
points(PC1[match(oddS,colnames(Xsurv))],PC3[match(oddS,colnames(Xsurv))] ,pch=pchCol[match(oddS,colnames(Xsurv))],col=2,cex=1.3)
abline(h=0,lty=2)
abline(v=0,lty=2)
legend(-100,60, uniqS, xpd = TRUE, horiz = TRUE, inset = c(0, 0),
       bty = "n", pch = c(16,17,3,4),col=tmp_col,cex=0.8)
