...
list2 = read.delim("../SigPathways_BRCA_Ord.txt",as.is=T,header=T)
comb_new = COMB[match(list2$ALLPaths,COMB$ALLPaths),]
comb_new$ALLPaths=as.character(comb_new$ALLPaths)
comb_new$Annot=as.character(comb_new$Annot)
missing = which(is.na(comb_new$ALLPaths))
comb_new$ALLPaths[missing] = list2$ALLPaths[missing]
comb_new$Annot[missing] = list2$Annot[missing]
...
pdf("heatmap_BRCAPathway_selectedPathways_ordered.pdf",height=15,width=8, useDingbats = F)
HM2= heatmap.2(PVALnew, trace="n", Rowv = F, Colv=F,col=bluered(20),breaks=seq(-3,3,by=0.3),
